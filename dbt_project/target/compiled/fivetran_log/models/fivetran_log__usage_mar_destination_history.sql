...
join_usage_mar as (
    ...
    select 
        ...
        round( cast(nullif(usage.credits_spent,0) * 1000000.0 as 
    numeric
) / cast(nullif(destination_mar.monthly_active_rows,0) as 
    numeric
), 2) as credits_spent_per_million_mar,
        round( cast(nullif(destination_mar.monthly_active_rows,0) * 1.0 as 
    numeric
) / cast(nullif(usage.credits_spent,0) as 
    numeric
), 0) as mar_per_credit_spent,
        round( cast(nullif(usage.dollars_spent,0) * 1000000.0 as 
    numeric
) / cast(nullif(destination_mar.monthly_active_rows,0) as 
    numeric
), 2) as amount_spent_per_million_mar,
        round( cast(nullif(destination_mar.monthly_active_rows,0) * 1.0 as 
    numeric
) / cast(nullif(usage.dollars_spent,0) as 
    numeric
), 0) as mar_per_amount_spent
